function [] = PreprocessDataSetYufei(topFocus, fullFocus, loc, imshow,num_each, num_test, matPATH,fileType, numFolders)
...
files_temp = [dir( fullfile(root,strcat('*',fileType)))];
...
for folders=1:numFolders
    for images=1:(num_each-num_test)
        organizedData.train{folders}(:,images) = data.train(:,images+(counter*(num_each-num_test)));
        organizedData.trainlabel{folders}(images) = data.train_label(images);
    end

    for images=1:num_test
        organizedData.test{folders}(:,images) = data.test(:,images+(counter*num_test));
...
%save(a,'data')
save([strcat('C:/Users/davis_000/Desktop/Composite Face Code/',matPATH)],'organizedData')
